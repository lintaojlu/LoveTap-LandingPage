...
chmod -R 775 $DEPLOY_PATH/stats  # 确保Node.js应用可以写入统计数据
...
# 13. 测试服务
echo "🧪 测试Node.js服务..."
curl -X POST "http://localhost:3000/record-stat?type=visit&date=$(date +%Y-%m-%d)" || echo "服务测试失败"
